...
pub const WsSession = struct { // for safe multithread use Pub/Sub, for save user's state (context)
    ...
    outbox: std.ArrayListUnmanaged(QueuedMessage), // messages queue that need to send to client with conn
    ...
    fn scheduleSendInternal(self: *WsSession, data: []const u8, is_binary: bool) !void {
        ...
        try self.outbox.append(self.allocator, .{ .data = msg_copy, .is_binary = is_binary });
        self.mutex.unlock();
...
pub const PubSub = struct {
    topics: std.StringHashMapUnmanaged(std.ArrayListUnmanaged(*WsSession)), // Topic Name -> List of Connections
    ...
    pub fn subscribe(self: *PubSub, topic: []const u8, session: *WsSession) !void {
        ...
        const entry = try self.topics.getOrPut(self.allocator, topic);
        ...
        try entry.value_ptr.append(self.allocator, session);
...
fn writer_task(session: *WsSession) !void { // send message in same thread with conn
    ...
    while (true) {
        ...
        const batch = try session.outbox.toOwnedSlice(session.allocator);
        session.mutex.unlock();
...
pub fn handle_upgrade(ctx: *const zzz.Context, user_handler: UserWsHandler, stack_size_writer: usize) !zzz.HTTP.Respond {
    ...
    const res = try zzz.websocket.upgrade(&ctx.socket, ctx.runtime, ctx.allocator, key, ext, header_buf.writer(ctx.allocator));
