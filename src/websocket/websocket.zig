...
pub const Conn = struct {
    ...
    pub fn send(self: Conn, data: []const u8) !void {
        ...
        defer buf.deinit(self.runtime.allocator);

        //try writeFrameHeader(buf.writer(), .text, data.len, true);
        try writeFrameHeader(buf.writer(self.runtime.allocator), .text, data.len, false);
        try buf.appendSlice(self.runtime.allocator, data);
    ...
    pub fn sendBinary(self: Conn, binary_data: []const u8) !void {
        ...
        try writeFrameHeader(buf.writer(self.runtime.allocator), .binary, binary_data.len, false);
        try buf.appendSlice(self.runtime.allocator, binary_data);
        _ = try self.socket.send_all(self.runtime, buf.items);
    }
    ...
    pub fn close(self: Conn, code: u16, reason: []const u8) !void {
        ...
        try writeFrameHeader(buf.writer(self.runtime.allocator), .close, payload_len, false); // close header

        try buf.writer(self.runtime.allocator).writeInt(u16, code, .big); // big-endian

        if (reason.len > 0) {
            try buf.appendSlice(self.runtime.allocator, reason);
        }
...
pub fn runLoop(conn: Conn, handler: Handler, allocator: Allocator) !void {
    ...
    var stash: std.ArrayListUnmanaged(u8) = .{}; // buffer for incompleted frames between recv calls
    defer stash.deinit(allocator);

    var fragment: std.ArrayListUnmanaged(u8) = .{}; // for fragmented ws msg - Opcode 0x0
    ...
    while (true) {
        ...
        try stash.appendSlice(allocator, read_buffer[0..n]); // save readed
          ...
          switch (opcode) {
            0x1 => { // Text
              ...
              if (fin) {
                ...
                try fragment.appendSlice(allocator, payload);
                //if (decompressed) |d| allocator.free(d);
              }
            },
            ...
            0x9 => { // Ping
              ...
              var pong_buf: std.ArrayListUnmanaged(u8) = .{};
              defer pong_buf.deinit(allocator);

              try writeFrameHeader(pong_buf.writer(allocator), .pong, pong_payload.len, false);
              try pong_buf.appendSlice(allocator, pong_payload);
